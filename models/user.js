...
// Consider importing ExpressError if you are using custom error handling
...
class User {
  ...
  static async register({username, password, first_name, last_name, phone}) {
    ...
    return result.rows[0];  // Fixed typo: changed row to rows
  }
  ...
  static async authenticate(username, password) { 
    ...
    if(user) {
      ...
      return valid ? user : false;  // Fixed typo: changed isValid to valid
    }
    return false;
  }
  ...
  static async updateLoginTimestamp(username) {
    ...
    if(!result.rows[0]) {
      throw new Error(`No such user: ${username}`);  // Consider using ExpressError for consistency
    }
  }
